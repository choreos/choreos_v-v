...
public class SampleSize implements Analyser {
	
	private double d;
	
	/**
	 * Calculates the appropriate sample size for the experiment.
	 * @param precision a degree of precision given in the same unit as the measurements 
	 */
	...
	public void analyse(List<ScalabilityReport> reports) throws Exception {
		for(ScalabilityReport report: reports) {
			for(Number key: report.keySet()) {
				...
				int n = sample.size();
				DescriptiveStatistics ds = DescriptiveStatisticsFactory.create(sample);
