...
public class WSClient {
	...
	public WSClient(String wsdl) throws XmlException, IOException,
		...
		if (!wsdl.startsWith("file")
				&& !HttpUtils.verifyIfUriReturns0kforGET(wsdl))
	 ...
	 * @param arguments
	 *            Variable number of string arguments to use in the request. The
	 *            arguments must be ordered according to the request wsdl. For
	 *            complex types with lots of arguments, it's better to use
	 *            {@link #request(String, Item)}
	 * @return An <code>Item</code> representing the request response.
	 * @throws InvalidOperationNameException
	 * @throws FrameworkException
	 */
	public Item request(String operationName, String... arguments)
			throws InvalidOperationNameException, FrameworkException {
	 ...
	 *            Name of the operation to invoke
	 * @param requestRoot
	 *            An Item representing the root of a tree that represents the
	 *            structure of the complex type arguments.
	 * @return An <code>Item</code> representing the request response.
	 * @throws InvalidOperationNameException
	 * @throws FrameworkException
	 */
	public Item request(String operationName, Item requestRoot)
			throws InvalidOperationNameException, FrameworkException {

		return makeRequest(operationName, Strategy.ITEM, requestRoot);
	}

	private String getDefaultRequestContent(String operationName)
			throws InvalidOperationNameException {
	...
	/**
	 * Makes the actual request to the web service. It uses a strategy passed as
	 * a parameter to determine which type of arguments to use to generate the
	 * final SOAP request envelope.
	 * 
	 * @param operationName
	 * @param strategy
	 * @param requestRoot
	 ...
	 * @throws InvalidOperationNameException
	 */
	private Item makeRequest(String operationName, Strategy strategy,
			Item requestRoot, String... parameters) throws FrameworkException,
			ParserException, InvalidOperationNameException {

		String defaultRequestContent = getDefaultRequestContent(operationName);

		String requestContent = null;
		if (strategy == Strategy.STRING) {
			requestContent = SoapEnvelopeHelper.generate(defaultRequestContent,
					parameters);
		} else if (strategy == Strategy.ITEM) {
			requestContent = new ItemBuilder().buildItem(defaultRequestContent,
					requestRoot);
		}
	...
	public Item getItemRequestFor(String operationName)
			throws InvalidOperationNameException, ParserException,
			EmptyRequetItemException {
		String defaultRequestContent = getDefaultRequestContent(operationName);
		Item item = new ItemParser().parse(defaultRequestContent);

		if (item.getChildren().size() == 0)
			throw new EmptyRequetItemException(
					"This operation has no parameter");

		return item;
	}

	public Item getItemResponseFor(String operationName)
			throws ParserException, XmlException, IOException, SoapUIException {
